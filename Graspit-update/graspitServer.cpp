...
ClientSocket::readClient()
{
  ...
  while ( canReadLine() ) {
    ...
    // date: 23 May 2013
    // comments: Added if clauses for commands autoGrasp, autoOpen
    //           loadObject, removeObject, rotateObject and 
    // 		 aomrGetJointAngles
    else if (*strPtr == "autoGrasp") {
      std::cout << "Command received: autoGrasp" << std::endl;
      strPtr++;
      autoGrasp();
    }
    else if (*strPtr == "autoOpen") {
      std::cout << "Command received: autoOpen" << std::endl;
      strPtr++;
      autoOpen();
    }
    else if (*strPtr == "loadObject") {
      std::cout << "Command received: loadObject" << std::endl;
      strPtr++;
      loadObject();
    }
    else if (*strPtr == "removeObject") {
      std::cout << "Command received: removeObject" << std::endl;
      strPtr++;
      removeObject();
    }
    else if (*strPtr == "rotateObject") {
      std::cout << "Command received: rotateObject" << std::endl;
      strPtr++;
      rotateObject();
    }
    else if (*strPtr == "aomrGetJointAngles") {
      std::cout << "Command received: aomrGetJointAngles" << std::endl;
      strPtr++;
      aomrGetJointAngles();
    }
    //-------------------END MODIFICATION------------------------
...
// date: 23 May 2013
// 
// autoGrasp method: Called when autoGrasp command is received 
// Calls autoGrasp command in Grasp menu and sends back message "OK"
void ClientSocket::autoGrasp()
{
      std::cout << "Entered ClientSocket::autoGrasp" << std::endl;
  QTextStream os(this);
  World *world = graspItGUI->getIVmgr()->getWorld();
  world->getCurrentHand()->autoGrasp(true);
  world->updateGrasps();
      std::cout << "Exiting ClientSocket::autoGrasp" << std::endl;
  os << "OK\n";
}

// autoOpen method: Called when autoOpen command is received 
// Calls autoOpen command in Grasp menu and sends back message "OK"
void ClientSocket::autoOpen()
{
      std::cout << "Entered ClientSocket::autoOpen" << std::endl;
  QTextStream os(this);
  World *world = graspItGUI->getIVmgr()->getWorld();
  world->getCurrentHand()->autoGrasp(true, -1.0);
  world->updateGrasps();
      std::cout << "Exiting ClientSocket::autoOpen" << std::endl;
  os << "OK\n";
}

// loadObject method: Called when loadObject command is received 
// Calls importBody method and sends back message "OK"
// Object file is assumed to be in VRML format and named obj.wrl
void ClientSocket::loadObject()
{
      std::cout << "Entered ClientSocket::loadObject" << std::endl;
  QTextStream os(this);
  World *world = graspItGUI->getIVmgr()->getWorld();
  world->importBody("GraspableBody","obj.wrl");
      std::cout << "Exiting ClientSocket::loadObject" << std::endl;
  os << "OK\n";
}

// removeObject method: Called when removeObject command is received 
// Calls destroyElement method on body and sends back message "OK"
// Removes all the graspable objects from world
void ClientSocket::removeObject()
{
      std::cout << "Entered ClientSocket::removeObject" << std::endl;
  QTextStream os(this);
  World *world = graspItGUI->getIVmgr()->getWorld();
  std::cout << "Body count: " << world->getNumBodies() << " Robot count: " << world->getNumRobots() << std::endl;
  // for some reason, destroyElement method fails if there are no robots in scene
  // that's why we have getNumRobots>0 condition
  if (world->getNumBodies() > 0 && world->getNumRobots() > 0)
  {
     std::cout << "Removing object" << std::endl;
     int i;
     for(i=0; i<world->getNumBodies(); i++)
     {
     	WorldElement* body = world->getBody(i);
     	// if body is an instance of GraspableBody, remove it
  	//std::cout << "B:" << body->inherits("Body") << " R: " << body->inherits("Robot") << " GB: " << body->inherits("GraspableBody") << std::endl;
  	if( body->inherits("GraspableBody") )
  	{
  	  world->destroyElement(body);
  	}
     }
     // get hand ready for next object, call autoOpen
     world->getCurrentHand()->autoGrasp(true, -1.0);
     world->updateGrasps();
  }
  else
  {
     std::cout << "No object to remove" << std::endl;
  }
      std::cout << "Exiting ClientSocket::removeObject" << std::endl;
  os << "OK\n";
}

// rotateObject method: Called when rotateObject command is received 
// Calculates new transformation based on current and calls moveTo method of body
// Rotates graspable body by 45 degrees around x axis
void ClientSocket::rotateObject()
{
    std::cout << "Entered ClientSocket::rotateObject" << std::endl;
  QTextStream os(this);
  World *world = graspItGUI->getIVmgr()->getWorld();
  std::cout << "Body count: " << world->getNumBodies() << " Robot count: " << world->getNumRobots() << std::endl;
  // for some reason, moveTo method fails if there are no robots in scene
  // that's why we have getNumRobots>0 condition
  if (world->getNumBodies() > 0 && world->getNumRobots() > 0 )
  {
     ...
     {
     	WorldElement* body = world->getBody(i);
     	// if body is an instance of GraspableBody, rotate it
  	if( body->inherits("GraspableBody") )
  	{
  	  transf rot;
	  transf tran;
	  rot.set(Quaternion(M_PI / 4.0, vec3::X), vec3(0,0,0));
	  tran = rot * body->getTran();
	  body->moveTo(tran, 50*Contact::THRESHOLD, M_PI/36.0);
  	}
      }
   }
   else
   {
     std::cout << "No object to rotate or no robots in scene" << std::endl;
   }
   std::cout << "Exiting ClientSocket::rotateObject" << std::endl;
   os << "OK\n";
}

// aomrGetJointAngles method: Called when aomrGetJointAngles command is received 
// Loads the object from obj.wrl, autoGrasps it, sends DOF vals and rotates object
// 45 deg around x axis and repeats these for 8 times. In effect, this method returns
// joint angles of grasps for different orientations of object. This functionality can
// can be achieved by calling other socket functions one after another, this method
// reduces this unnecessary communication cost by doing all of them in a single method
void ClientSocket::aomrGetJointAngles()
{
    std::cout << "Entered ClientSocket::aomrGetJointAngles" << std::endl;
  QTextStream os(this);
  int i;
  World *world = graspItGUI->getIVmgr()->getWorld();
  
  // get robot hand
  std::cout << "Getting robot" << std::endl;
  Robot* robot = NULL; 
  if (world->getNumRobots() > 0 )
  {
     //there should be only one robot
     robot = world->getRobot(0);
  }
  ...
  std::cout << "Importing object" << std::endl;
  WorldElement* body = world->importBody("GraspableBody","obj.wrl");
  if( body == NULL )
  {
    os << "FAIL: Cannot load object\n";
    return;
  }
  int j;
  vec3 axis[] = {vec3::X, vec3::Y, vec3::Z};
  int rotAxis;
  for( rotAxis = 0; rotAxis<3; rotAxis++)
  {
	  ...
	  for(j=0; j<8; j++){
			// autoGrasp
			std::cout << "Auto Grasping" << std::endl;
			world->getCurrentHand()->autoGrasp(true);
			world->updateGrasps();
		
			// send joint angles
			std::cout << "Sending DOF Vals" << std::endl;
			for (i=0;i<robot->getNumDOF();i++)
			  os << robot->getDOF(i)->getVal() << "\n";
		
			// disable collision, we do this because sometimes the 
			// hand gets stuck and autoOpen cannot open the hand
			// which prevents us from rotating the object
			// however, if we disable collisions, autoOpen works
			// fine
	  		std::cout << "Toggling collisions" << std::endl;
	  		world->toggleAllCollisions(false);
	  	
			// autoOpen
			std::cout << "Auto Opening" << std::endl;
			world->getCurrentHand()->autoGrasp(true, -1.0);
			world->updateGrasps();
			...
			transf rot;
			//transf tran;
			rot.set(Quaternion(j * M_PI / 4.0, axis[rotAxis]), vec3(0,0,0));
			//tran = rot * body->getTran();
			body->moveTo(rot, 50*Contact::THRESHOLD, M_PI/36.0);
		}
  }	  
  // remove object
  std::cout << "Removing Object" << std::endl;
  ...
  world->getCurrentHand()->autoGrasp(true, -1.0);
   ...
   std::cout << "Exiting ClientSocket::aomrGetJointAngles" << std::endl;
}
